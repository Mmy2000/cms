...
class ExpectedStampExcelService:
    """
    ...
    def export_basic_report(queryset):
        ...
        Includes sector, date, value, copies, rate, and total.
        """
    ...
    def export_formatted_report(queryset):
        ...
        for s in queryset:
            ws.cell(row=row_num, column=1, value=s.sector.name)
            ws.cell(
                row=row_num,
                column=2,
    ...
    @staticmethod
    def export_sector_summary_report(queryset):
        """
        Export Excel report grouped by sector with subtotals.
        Shows summary statistics per sector.
        """

        wb = openpyxl.Workbook()
        ws = wb.active
        ws.title = "ملخص القطاعات"
        ...
        # Headers
        headers = ["القطاع", "عدد الفواتير", "إجمالي النسخ", "إجمالي الدمغة"]
        ...
        # Get grouped data
        grouped_data = (
            queryset.values("sector__name", "sector_id")
            .annotate(
                total_amount=Sum("d1"),
        ...
        for item in grouped_data:
            ws.cell(row=row_num, column=1, value=item["sector__name"])
            ws.cell(
                row=row_num,
                column=2,
                value=queryset.filter(sector_id=item["sector_id"]).count(),
            )
            ...
            total_invoices += queryset.filter(sector_id=item["sector_id"]).count()
